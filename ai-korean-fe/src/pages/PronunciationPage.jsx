...
const PronunciationPage = () => {
  ...
  const handleStop = () => {
    ...
    mediaRecorderRef.current.onstop = async () => {
      ...
      try {
        const res = await fetch(`${backendUrl}/api/pronunciation/evaluate`, { method: "POST", body: formData });
  ...
  function splitHangulSyllable(syllable) {
    const CHOSEONG = 0x1100, JUNGSEONG = 0x1161, JONGSEONG = 0x11a7;
    ...
    const chars = [String.fromCharCode(CHOSEONG + L), String.fromCharCode(JUNGSEONG + V)];
  ...
  function mapSyllableToPhonemeScoreWithHangul(text, phonemes) {
    ...
    for (const syllable of text) {
      ...
      const avgScore = syllablePhonemes.reduce((sum, p) => sum + (p.score || 0), 0) / (syllablePhonemes.length || 1);
  ...
  return (
    <div className="container mt-4">
      <h2>Luyện phát âm - {mode === "level" ? `Cấp độ ${level}` : `Chủ đề: ${topic || "..."}`}</h2>
      ...
      <div className="mb-4">
        {[1, 2, 3, 4, 5].map((lv) => (
          <button
            key={lv}
            className={`btn me-2 ${mode === "level" && level === lv ? "btn-primary" : "btn-outline-primary"}`}
      ...
      {mode === "topic" && !topic && (
        <div className="mb-4">
          ...
          {Object.keys(dataByTopic).map((t) => (
            <button key={t} className="btn btn-outline-secondary me-2 mb-2" onClick={() => handleSelectTopic(t)}>
              {t}
            </button>
          ))}
        </div>
      ...
      {current.text && (
        <div className="card p-3">
          <h4>{current.text}</h4>
          <p><strong>Chú thích:</strong> {current.notes}</p>
          <p><strong>Phiên âm:</strong> {current.ipa || current.pronunciation}</p>

          <div className="mt-3">
            <button className="btn btn-primary me-2" onClick={playSample} disabled={isPlaying}>
              🔊 Nghe mẫu
            </button>
            <button
          ...
          {currentEvaluation?.detail && (
            <div className="mt-3 p-3 border rounded">
              ...
              <p><strong>Điểm trung bình:</strong> {((currentEvaluation.score ?? 0) * 100).toFixed(0)}%</p>
              <div style={{ fontSize: "1.5rem", lineHeight: "2rem", position: "relative" }}>
                {mapSyllableToPhonemeScoreWithHangul(current.text, currentEvaluation.detail).map((s, idx) => {
                  const { hangulLetters, phonemes, avgScore } = s;
                  let color = avgScore >= 0.8 ? "green" : avgScore >= 0.6 ? "orange" : "red";
                  return (
                    <span key={idx} style={{ color, marginRight: "0.05rem", position: "relative" }}>
                      ...
                      {hoveredWordIndex === idx && phonemes.length > 0 && (
                        <div style={{
                          ...
                          background: "white", border: "1px solid #ccc", padding: "4px 8px",
                          borderRadius: "4px", fontSize: "0.9rem", zIndex: 10, whiteSpace: "nowrap"
                        }}>
                          {phonemes.map((p, i) =>
                            `${hangulLetters[i]?.hangul || "?"} (${p.phoneme || "?"} (${((p.score ?? 0) * 100).toFixed(0)}%))`
                          ).join(" ")}
                        </div>
                      )}
                    </span>
          ...
          <div className="mt-4">
            ...
            <button
              ...
              onClick={() => setSelectedIndex((prev) => Math.min(sampleData.length - 1, prev + 1))}
