# core/ctc_gate.py
# CTC-only gate & confidences for Korean (NFD/Compat/Syllable aware)
...
# ==== Preload helpers ====
def set_ctc_device(device: Optional[str] = None):
    """(tuỳ chọn) ghi đè device khi preload từ app.py."""
    global DEVICE
    if device:
        DEVICE = device

def set_ctc_model_name(model_name: Optional[str] = None):
    """(tuỳ chọn) ghi đè model name khi preload từ app.py."""
    global MODEL_NAME
    if model_name:
        MODEL_NAME = model_name

def preload_ctc(model_name: Optional[str] = None, device: Optional[str] = None):
    """
    Gọi trước ở startup để load W2V2ForCTC + tokenizer/processor vào RAM/GPU.
    Không đổi logic _init() hiện có (giữ lazy cho code cũ), chỉ kích hoạt sớm.
    """
    if model_name: set_ctc_model_name(model_name)
    if device:     set_ctc_device(device)
    _init()   # sẽ load processor + model + vocab mode

def ctc_dummy_forward(sample_rate: int = 16000, sec: float = 0.6):
    """
    Chạy 1 forward ngắn để warm-up kernel/cuda graph.
    Không cần chính xác nội dung; chỉ để giảm độ trễ request đầu.
    """
    _init()
    import torch
    T = int(sample_rate * sec)
    wav = torch.zeros(1, T)  # silence
    wav = wav.to(next(_model.parameters()).dtype).cpu()  # model chạy FP32; giữ CPU -> processor trả input_values float
    with torch.inference_mode():
        inputs = _processor(wav.squeeze(0), sampling_rate=sample_rate, return_tensors="pt", padding=False)
        input_values = inputs.input_values.to(DEVICE)
        _ = _model(input_values).logits  # bỏ
