...
def evaluate(audio_file: str, reference_text: str, textgrid_file: str):
    try:
        # 1) ASR
        if not model:
            raise RuntimeError("ASR model not loaded")
        result = model.transcribe(audio_file, language='ko', fp16=use_gpu)
        ...
        user_phonemes = text_to_phonemes_mfa(user_transcript, mfa_dict) or []
...
if __name__ == "__main__":
    import json
    output = evaluate(
        audio_file="./data_model_first/model_first/level1_pronunciation_data_0.wav",
        reference_text="가",
        textgrid_file="./data_model_first/align_output/level1_pronunciation_data_0.TextGrid",
    )
