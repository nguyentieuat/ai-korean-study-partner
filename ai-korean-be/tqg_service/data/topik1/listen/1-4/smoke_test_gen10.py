...
# ===== Gen nhiều câu + lưu JSONL =====
import time, hashlib, json

# (A) sinh 1–2 câu hội thoại/đoạn ngắn để hỗ trợ FULL fallback
def generate_passage(hint="일상 대화", sample=True):
    sys = ("당신은 TOPIK I용 짧은 지문/대화를 만듭니다. "
           "한국어 한 단락(1~2문장)만 출력하세요. 다른 텍스트/따옴표/JSON 금지.")
    ...
    out = gen.generate(
        **ids,
        generation_config=GenerationConfig(
            do_sample=bool(sample), temperature=0.8 if sample else None, top_p=0.9 if sample else None,
            repetition_penalty=1.1, max_new_tokens=80, pad_token_id=tok.pad_token_id
        )
    )
...
# (B) FULL robust: thử FULL, nếu thiếu passage_ko → sinh passage rồi FROM_PASSAGE
def generate_full_robust(hint="일상 대화", sample=True):
    item, ok, _, _ = generate_once("FULL", hint=hint, sample=sample)
    if item and item.get("passage_ko"):
        return item, True, []
    passage = generate_passage(hint=hint, sample=sample)
    item2, ok2, reasons2, _ = generate_once("FROM_PASSAGE", passage=passage, sample=sample)
    if item2:
        item2["passage_ko"] = passage
    return item2, ok2, reasons2

# (C) chữ ký để khử trùng lặp
def item_sig(it: dict):
    p = it.get("passage_ko","").strip()
    choices = it.get("choices",{})
    key = "|".join([p, it.get("prompt_ko","").strip()] + [choices.get(k,"").strip() for k in ("A","B","C","D")])
    return hashlib.md5(key.encode("utf-8")).hexdigest()

# (D) thu thập N câu
def collect_n(task="FULL", n=10, hints=None, passages=None, sample=True, require_valid=True, max_tries=None):
    """
    task: "FULL" hoặc "FROM_PASSAGE"
    hints: danh sách chủ đề để xoay vòng khi FULL (vd: ["날씨","식사","쇼핑","교통"])
    passages: list các passage có sẵn nếu muốn dùng FROM_PASSAGE cố định
    require_valid: chỉ nhận item pass validator
    """
    items, seen = [], set()
    tries = 0
    if max_tries is None: max_tries = n * 8
    ...
    while len(items) < n and tries < max_tries:
        ...
        if task == "FULL":
            ...
            i_hint += 1
            it, ok, reasons = generate_full_robust(hint=hint, sample=sample)
        else:
            # FROM_PASSAGE
            if passages and len(passages) >= n:
                passage = passages[len(items)]
            else:
                # không có passage sẵn → tự sinh
                hint = (hints[i_hint % len(hints)] if hints else "일상 대화")
                i_hint += 1
                passage = generate_passage(hint=hint, sample=sample)
            it, ok, reasons, _ = generate_once("FROM_PASSAGE", passage=passage, sample=sample)

        if it is None:
            continue
        # chuẩn hoá + validate (đã làm trong generate_once), ở FULL robust it đã có passage_ko
        ok2, reasons2 = validate_item(it)
        ok_final = ok and ok2 if require_valid else True
        sig = item_sig(it)
        if ok_final and sig not in seen:
            items.append(it)
            seen.add(sig)
        time.sleep(0.05)  # tránh spam GPU
    return items
...
def save_jsonl(items, path):
    ...
    print("Saved:", path, "| n =", len(items))

# ======= Ví dụ chạy: gen 10 câu =======
# danh sách chủ đề để đa dạng
hint_pool = ["날씨와 약속","식사와 주문","쇼핑과 가격","교통과 길찾기","가족과 취미","학교와 수업","직장과 일정"]

# 1) FULL (tự sinh passage + MCQ) – 10 câu
full_10 = collect_n(task="FULL", n=10, hints=hint_pool, sample=True, require_valid=True)
save_jsonl(full_10, "./topik_full_10.jsonl")

# 2) FROM_PASSAGE (nếu có sẵn passage) – 10 câu
# Nếu chưa có, nó sẽ tự sinh passage từ hint_pool
fp_10 = collect_n(task="FROM_PASSAGE", n=10, hints=hint_pool, sample=True, require_valid=True)
save_jsonl(fp_10, "./topik_from_passage_10.jsonl")

# xem nhanh 2 item đầu
print("\nFULL[0..1]:")
for ex in full_10[:2]:
