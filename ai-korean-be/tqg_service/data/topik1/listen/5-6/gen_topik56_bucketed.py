# %%writefile gen_topik56_bucketed.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
USE_4BIT     = True   # cần GPU, BitsAndBytes 4-bit

# Tối ưu tốc độ/độ ổn định JSON
MAX_NEW_TOKENS      = 160        # 128~160; 160 an toàn hơn cho JSON dài
TEMPERATURE         = 0.6
TOP_P               = 0.9
REPETITION_PENALTY  = 1.08

NUM_TOTAL           = 1000       # tổng (4 bucket x 250)
PER_BUCKET          = NUM_TOTAL // 4
SAVE_PATH           = "/kaggle/working/topik56_bucketed_1000.jsonl"
BATCH_SIZE          = 12         # 8–12 (T4); 16–24 (A100)
CHECKPOINT_EVERY    = 100        # ghi file mỗi 100 mẫu hợp lệ
TOPICS = (
...
def load_model():
    ...
    # pad/eos & padding side cho decoder-only
...
def generate_batch(tok, model, topics: List[str], qno: int, speaker: str) -> List[str]:
    # Tính ngân sách input dựa vào context window
    ctx = getattr(getattr(model, "config", None), "max_position_embeddings", 8192)
    input_max_len = max(256, ctx - MAX_NEW_TOKENS - 16)  # chừa chỗ output + margin
...
# ================== BUCKET DRIVER ==================
def run_bucket(tok, model, qno: int, speaker: str, n_target: int,
               topics, save_path, batch_size=12, checkpoint_every=100):
    import os
    ...
    def _flush():
        ...
        # Đặt file tạm trong cùng thư mục của save_path (tránh lỗi khác volume)
        base = os.path.basename(save_path)
        tmp = os.path.join(dirpath if dirpath else ".", f".{base}.part")
        ...
        # Append-atomic: mở tmp đọc, append vào file chính rồi xóa tmp
        with open(tmp, "r", encoding="utf-8") as fin, open(save_path, "a", encoding="utf-8") as fout:
            for line in fin:
                fout.write(line)
            fout.flush()
    ...
    while count < n_target:
        bs = min(batch_size, n_target - count)
        batch_topics = [next(topic_cycle) for _ in range(bs)]
        bodies = generate_batch(tok, model, batch_topics, qno=qno, speaker=speaker)
        ...
        for body in bodies:
            ...
            if not ok: 
                continue
            # xác minh đúng bucket
            if obj.get("question_no") != qno: 
                continue
            if obj.get("dialogue", [{}])[0].get("speaker") != speaker:
                continue
            ...
            if count % 50 == 0:
                print(f"[{qno}-{speaker}] {count}/{n_target}")
            if len(buffer) >= checkpoint_every:
                _flush()

    _flush()
    print(f"[{qno}-{speaker}] DONE {count}/{n_target}")
...
if __name__ == "__main__":
    ...
    # Chạy 4 bucket: mỗi bucket 250 mẫu
    for qno, spk in [(5,"남자"), (5,"여자"), (6,"남자"), (6,"여자")]:
        run_bucket(
            tok, model, qno, spk, PER_BUCKET,
            topics=TOPICS,
            save_path=SAVE_PATH,
    ...
    # Thống kê nhanh
    stats = {"answer": Counter(), "qno": Counter(), "speaker": Counter()}
    try:
        with open(SAVE_PATH, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    continue
                stats["answer"][r.get("answer")] += 1
                stats["qno"][int(r.get("question_no", 0))] += 1
                stats["speaker"][r.get("dialogue", [{}])[0].get("speaker")] += 1
    except Exception as e:
        print("[WARN] Stats error:", e)

    print("[DIST] answer:", dict(stats["answer"]))
    print("[DIST] qno:", dict(stats["qno"]))
    print("[DIST] speaker:", dict(stats["speaker"]))
    print("ALL DONE:", SAVE_PATH)
